...
注：图中 MAC 是硬件的真实地址，实际上已经通过软件方法修改了其他设备看到的地址。
...
使用开放的 WiFi 网络是危险的行为，除了明文流量容易被盗取外，也容易受到伪装等攻击。我们应当尽量避免连接到这样的网络，如果一定要，需要使用 VPN 等手段加密自己的流量。加密的网络虽然无法被直接侦听，但如果是预共享密钥网络，也无法避免密钥泄露导致的一系列攻击，需要额外的机制（类似 802.1X 这样的每客户端验证）来预防。