...
在 AS 4538 中，至少有 57137 个 IP 地址（约占全部 IP 地址的 0.33%）能够向公众提供正确的 DNS 解析服务；另外，有 35 个 IP 地址响应请求，但返回错误的结果；还有 2058 个 IP 地址返回的数据报格式错误（可能不是 DNS 协议）。
...
该小工具有一个参数需要调节，即 $t_{threshold}$。为了降低返回给用户的延迟，需要降低 $t_{threshold}$，但为了确保在超时之前接收到劫持响应包，则需要增加 $t_{threshold}$，于是这里需要一个折中，需要反复调节和测试。本节实验发现 $t_{threshold} = 100ms$ 效果良好。
