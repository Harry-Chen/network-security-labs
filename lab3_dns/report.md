...
- 2015011278 谭闻德：整理 AS4538 宣告的前缀列表、统计单权威服务器的域名列表、探测被防火墙劫持的域名列表、开发与测试用于绕过劫持的小工具、撰写测试分析报告
- 2016010981 陈晟祺：完成 `edu.cn.` 权威服务器的发现、完成 AS4538 解析服务器的发现与统计、测试权威服务器的区域传输、测试权威服务器的动态更新、撰写测试分析报告
...
令人惊讶的是，脚本从 68 台服务器上成功得到了返回结果，结果存储于 `edu_cn_ns/axfr_result` 目录中。其中，不同的域名共有 59 个，占有效域名的 0.93%。具体分析发现，有 51 个域名不只配置有一台权威服务器，其中 46 个配置有两台，5 个配置有三台。一种合理的解释为，管理员或许是为了进行主从复制而打开了主域名服务器的区域传输功能，但是却没有正确配置访问权限控制。其他 8 个域名各自仅有一台权威服务器，却也开启了区域传输功能，我们无法给出合理的解释。
...
在 AS4538 中，至少有 57137 个 IP 地址（约占全部 IP 地址的 0.33%）能够向公众提供正确的 DNS 解析服务；另外，有 35 个 IP 地址响应请求，但返回错误的结果；还有 2058 个 IP 地址返回的数据报格式错误（可能不是 DNS 协议）。
...
由于某些原因，某些域名会被防火墙劫持，体现为 DNS 解析被抢答。下面几张图片展示了几类不同的情况。

首先，是对一个未被劫持的域名进行正常解析请求：

![8.8.8.8_not_polluted](8.8.8.8_not_polluted.png)

可以看到，收到了正常的响应（2），延迟大约 331 毫秒。

然后，解析 youtube.com（这是一个已知被劫持的域名）时的报文则如图所示：

![8.8.8.8_polluted](8.8.8.8_polluted.png)

可以看到，共收到了三个响应包。事实上，只有一个正常的响应（6），延迟大约 165 毫秒，同时出现了两个额外的响应（4 和 5），且延迟都很短，只有几毫秒。

此外，向一个不存在的境外服务器发送未被劫持的域名的解析请求，如图所示：

![1.2.3.4_not_polluted](1.2.3.4_not_polluted.png)

没有收到响应。

最后，向一个不存在的境外服务器发送 youtube.com 的解析请求，如图所示：

![1.2.3.4_polluted](1.2.3.4_polluted.png)

可以看到，出现了两个额外的响应（2 和 3），且延迟都很短，只有几毫秒。

了解 DNS 劫持具体症状后，本小节首先从 Alexa 网站的 Alexa Top Sites 抓取了全球排名前 500 的网站域名列表（`dns_pollution/alexa/global.txt`），以及计算机类排名前 500 的网站域名列表（`dns_pollution/alexa/computer.txt`）。然后，根据上面的分析，本小节设计了如下两个算法，编写脚本（`dns_pollution/detect_dns_pollution.py`）来批量探测列表中域名是否被防火墙劫持，同时统计劫持响应包的 TTL 值以及劫持响应包中 IP 地址。

利用一个不存在的境外服务器，可以设计出第一种算法。

```
探测算法一（对应脚本中 test1 函数）：
  输入：域名
  算法流程：
  1. 向一个不运行 DNS 服务的服务器发送该域名的 DNS 查询请求
  2. 持续等待并接收一个或多个响应，持续 100 毫秒
  3. 若 2. 收到了任何响应，则此域名被劫持，记录劫持情况，统计劫持响应包的 TTL 值以及劫持响应包中 IP 地址
```

在互联网中，丢包的情况相对常见，由于丢包只会造成响应包减少，容易知道该算法在出现丢包的情况下不会出现误报。由于防火墙在劫持时会发出两个伪造的响应包，因此，在出现丢包的情况下，漏报的概率也相对较小。

该算法的优点在于，由于劫持响应包返回较快，因此 2. 等待时间可以较短，例如仅设置为 100 毫秒。

此外，利用一个已知的 DNS 解析服务器，可以设计出第二种算法。

```
探测算法二（对应脚本中 test2 函数）：
  输入：域名
  算法流程：
  1. 向 8.8.8.8 解析服务器发送该域名的 DNS 查询请求
  2. 持续等待并接收一个或多个响应，持续 500 毫秒
  3. 若 2. 收到的响应数多于一个，则此域名被劫持，记录劫持情况，统计劫持响应包的 TTL 值以及劫持响应包中 IP 地址
```

类似地，在出现丢包的情况下，该算法不会出现误报，漏报的概率也相对较小。
...
探测结果原始数据存储于 `dns_pollution/{global,computer}_pollute_report.{txt,ip.csv,ttl.csv}`。

探测发现，全球排名前 10 的域名中，有 3 个被劫持，占 30%；全球排名前 100 的域名中，有 38 个被劫持，占 38%；全球排名前 500 的域名中，有 102 个被劫持，占 20.4%。特别地，102 个被劫持的域名中有 50 个为 Google 相关网站。经过简单的浏览，其他被劫持的网站大致分为新闻类、社交类、即时通讯类、博客类、文件共享类和色情视频类等。其中，大多数网站为用户提供准确的信息或允许用户自由地表达其言论。

在计算机类中，排名前 10 的域名中，有 5 个被劫持，占 50%；排名前 100 的域名中，有 20 个被劫持，占 20%；排名前 500 的域名中，有 33 个被劫持，占 6.6%。

此外，统计劫持响应包后，本节获得了 466 个不同的虚假响应 IP 地址，其中最常出现的 5 个为：
...
该小工具有一个参数需要调节，即 $t_{threshold}$。为了降低返回给用户的延迟，需要降低 $t_{threshold}$，但为了确保在超时之前接收到劫持响应包，则需要增加 $t_{threshold}$，于是这里需要一个折中，需要反复调节和测试。本节实验发现 $t_{threshold} = 100ms$ 时效果良好。
...
在中国大陆 DNS 劫持分析与绕过一节，本实验发现世界上相当多的著名网站的域名都在中国大陆遭到污染。从这一点就可以看出，中国大陆的互联网环境（包括政策上的以及部分技术上的）事实上是非常恶劣的，不过也正因为这个广泛部署的大型防火墙，很多相关研究能够开展，很多高质量学术论文得以发表。
