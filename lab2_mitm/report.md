...
由于进行欺骗后，攻击者事实上成为了受害者和网关间的一个“透明网关”，所以需要打开 Linux 的包转发功能，即以 root 身份执行：
...
通过撰写脚本对 mitmproxy 捕获的流量进行处理和分析（请见附录2），我们可以自动化地抓取HTTP请求中的用户名和密码，以及修改HTTP响应的内容。

其中重要的步骤是所谓的“降级攻击”，即将网页中表单的 target 更改为 HTTP 协议，这样就能从请求中抓取用户名和密码数据。我们没有选择替换所有的 `https://` 为 `http://`，而是对于一部分校内站点的表单做了特判。这是由于某些站点指定了端口，简单的全文替换会失败（当然，也不难进行篡改）。如下图，攻击者能够抓到受害者登录网络学堂的密码：
...
并且，可以从上面图中的日志里看到，我们还修改了HTTP响应中的图片内容，将彩色的图片转换为纯黑白点阵图片。一些样例如下面几张图所示：
...
访问未加密的HTTP网站是一种十分危险的行为，只要攻击者能够捕获到流量（如上一个实验的开放 WiFi 监听和这个实验的 ARP 欺骗），受害者就会在毫无知觉的情况下泄露自己的信息。同时，降级攻击的存在也说明了全站 HTTPS（甚至全站 HSTS） 是十分必要的。但即便如此，如果没有一定的安全意识，随意信任未经浏览器验证的证书，那么 HTTPS 也只是徒劳。
